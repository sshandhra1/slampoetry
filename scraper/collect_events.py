...
SEARCHES = [("San Francisco","San Francisco, CA"),("Oakland","Oakland, CA"),
    ("Berkeley","Berkeley, CA"),("San Jose","San Jose, CA"),("San Rafael","San Rafael, CA")]
TERMS = ["poetry","slam poetry","open mic poetry","spoken word"]
COORDS = {"San Francisco":(37.7749,-122.4194),"Oakland":(37.8044,-122.2712),
    "Berkeley":(37.8716,-122.2727),"San Jose":(37.3382,-121.8863),"San Rafael":(37.9735,-122.5311)}
TYPES = {"slam":["slam"],"open_mic":["open mic","open-mic","spoken word"],
    "workshop":["workshop","class","writing"],"festival":["festival","gala"]}

def classify(name):
    nl = name.lower()
    for t,kws in TYPES.items():
        if any(k in nl for k in kws): return t
    return "reading"
...
def fetch(city, loc, term):
    ...
    for ev in evs:
        ...
        ts = datetime.fromisoformat(sl).strftime("%-I:%M %p") if sl and "T" in sl else None
        vo = ev.get("venue") or {}
        ad = vo.get("address",{})
        lat = vo.get("latitude"); lng = vo.get("longitude")
        if lat and lng: lat,lng = float(lat),float(lng)
        else: lat,lng = COORDS.get(city,(37.7749,-122.4194))
        eid = "eb-" + str(ev.get("id",""))
        out.append({"external_id":eid,"name":name,"venue":vo.get("name",city),
            "city":ad.get("city",city),"state":"CA","region":"west",
            "lat":lat,"lng":lng,"type":classify(name),
            "date":ds,"time":ts,"price":price(ev),"url":ev.get("url",""),"source":"eventbrite"})
